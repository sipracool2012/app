...
# Centralized uploads directory (relative to backend root)
...
async def upload_passport(
    ...
    if not success:
        file_path = UPLOAD_DIR / f"{application_id}_passport_{file.filename}"
...
async def upload_photo(
    ...
    if not success:
        file_path = UPLOAD_DIR / f"{application_id}_photo_{file.filename}"
        with open(file_path, "wb") as buffer:
            shutil.copyfileobj(file.file, buffer)
        s3_key = f"local:{file_path}"
